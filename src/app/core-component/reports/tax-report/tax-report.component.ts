...
export class TaxReportComponent {
  ...
  public tableData2: Array<taxReport> = [];
  public pageSize = 10;
  ...
  constructor(
    ...
    this.data.getTaxReport2().subscribe((apiRes: apiResultFormat) => {
      this.totalData = apiRes.totalData;
      this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
        if (this.router.url == this.routes.taxReport) {
  ...
  private getTableData2(pageOption: pageSelection): void {
    this.data.getTaxReport2().subscribe((apiRes: apiResultFormat) => {
      ...
      this.dataSource = new MatTableDataSource<taxReport>(this.tableData2);
      this.pagination.calculatePageSize.next({
        totalData: this.totalData,
        pageSize: this.pageSize,
